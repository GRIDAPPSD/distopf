...
class TestDualExtractionHighLevel:
    ...
    def test_run_opf_with_duals_false(self):
        ...
        assert not hasattr(result.raw_result, "dual_power_balance_p")

    def test_run_opf_duals_accessible_via_raw_result(self):
        """Test that duals are accessible via result.raw_result"""
        case = opf.create_case(opf.CASES_DIR / "csv" / "ieee13")
        result = case.run_opf(objective="loss", backend="pyomo", duals=True)
    ...
    def test_dual_dataframe_has_correct_columns(self):
        ...
        actual_columns = set(result.raw_result.dual_power_balance_p.columns)
    ...
    def test_dual_values_are_numeric(self):
        ...
        assert pd.api.types.is_numeric_dtype(
            result.raw_result.dual_power_balance_p["dual"]
        )
        assert pd.api.types.is_numeric_dtype(
            result.raw_result.dual_power_balance_q["dual"]
        )
        assert pd.api.types.is_numeric_dtype(
            result.raw_result.dual_voltage_drop["dual"]
        )
    ...
    def test_dual_dataframe_no_null_values(self):
        ...
        assert not result.raw_result.dual_power_balance_p.isnull().any().any()
        assert not result.raw_result.dual_power_balance_q.isnull().any().any()
        assert not result.raw_result.dual_voltage_drop.isnull().any().any()
    ...
    def test_dual_id_name_consistency(self):
        ...
        df = result.raw_result.dual_power_balance_p
        # Each unique id should have consistent name
    ...
    def test_dual_time_index_consistency(self):
        ...
        df = result.raw_result.dual_power_balance_p
        # All time values should be non-negative integers
        assert (df["t"] >= 0).all()
    ...
    def test_dual_values_reasonable_magnitude(self):
        ...
        # Duals should be finite (not inf or nan)
        assert (
            result.raw_result.dual_power_balance_p["dual"]
    ...
    def test_get_dual_matches_pre_extracted(self):
        """Test that get_dual() matches pre-extracted duals"""
        case = opf.create_case(opf.CASES_DIR / "csv" / "ieee13")
        result = case.run_opf(objective="loss", backend="pyomo", duals=True)
        ...
        # Get via attribute
        duals_attr = result.raw_result.dual_power_balance_p
    ...
    def test_get_all_duals_includes_pre_extracted(self):
        ...
        # Should include power_balance_p
        assert "power_balance_p" in all_duals
        pd.testing.assert_frame_equal(
            all_duals["power_balance_p"], result.raw_result.dual_power_balance_p
        )
    ...
    def test_multiple_runs_with_duals(self):
        ...
        # Both should have duals
        assert hasattr(result1.raw_result, "dual_power_balance_p")
        assert hasattr(result2.raw_result, "dual_power_balance_p")

        # Results should be identical
        pd.testing.assert_frame_equal(
            result1.raw_result.dual_power_balance_p,
            result2.raw_result.dual_power_balance_p,
        )
