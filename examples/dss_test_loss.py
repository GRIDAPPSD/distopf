...
for _dir in dirs:
    print(_dir)
    for mult in np.linspace(0, 1, 11):
        ...
        v_diff = v_df.copy()
        v_diff.loc[:, ["a", "b", "c"]] = (
            v_df.loc[:, ["a", "b", "c"]] - dss.v_solved.loc[:, ["a", "b", "c"]]
        ).abs()
        v_rdiff = v_diff.loc[:, ["a", "b", "c"]] / dss.v_solved.loc[:, ["a", "b", "c"]]
        v_diff = v_diff.loc[:, ["a", "b", "c"]]
        print(f"{mult:.2f}: V error %: {v_rdiff.max().max():.3%}")
        loss_list.append(
            {
                "name": names[_dir.parent.name],
                "Load Multiplier": mult,
                r"$\%P_{loss}$": s_loss[0],
                r"$\%Q_{loss}$": s_loss[1],
                r"$\%S_{loss}$": np.sqrt(s_loss[0] ** 2 + s_loss[1] ** 2),
                r"$\%V_{error}$": v_rdiff.max().max() * 100,
                r"$V_{error}$": v_diff.max().max(),
            }
        )
loss_df = pd.DataFrame(loss_list)
fig = px.line(loss_df, x=r"$\%P_{loss}$", y=r"$\%V_{error}$", color="name")
fig.update_traces(mode="lines+markers")
fig.show()
fig = px.line(loss_df, x="Load Multiplier", y=r"$\%V_{error}$", color="name")
fig.update_traces(mode="lines+markers")
fig.show()
# fig = px.line(loss_df, x="%Qloss", y="%V_err", color="name")
# fig.update_traces(mode="lines+markers")
# fig.show()
# fig = px.line(loss_df, x="%Sloss", y="%V_err", color="name")
# fig.show()
