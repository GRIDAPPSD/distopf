...
def create_lindist_model(
    ...
    registry.add(CapacitorProvider())
...
def add_constraints(
    ...
    # functions install the compatibility adapter so existing models retain all
    # device injections rather than accidentally receiving an empty registry.
    ...
    # skip registration when a caller supplied an equivalent namespace.
    ...
    if device_providers is not None:
        ...
        provider_registry.add(LoadProvider())
        provider_registry.add(GeneratorProvider())
        provider_registry.add(CapacitorProvider())
