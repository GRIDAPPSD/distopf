...
def _create_load_parameters(m: pyo.ConcreteModel, case: Case) -> None:
    ...
    for _, row in case.bus_data.iterrows():
        for phase in row.phases:
            ...
            load_p_data[(row.id, phase)] = p_load
            load_q_data[(row.id, phase)] = q_load
...
def _create_generator_parameters(m: pyo.ConcreteModel, case: Case) -> None:
    ...
    for _, row in case.gen_data.iterrows():
        for phase in row.phases:
            ...
            p_gen_data[(row.id, phase)] = p_gen
            q_gen_data[(row.id, phase)] = q_gen
...
def _create_v_swing_parameters(m: pyo.ConcreteModel, case: Case) -> None:
    ...
    for _, row in swing_buses.iterrows():
        for phase in row.phases:
            ...
            v_swing_data[(row.id, phase)] = v_swing
...
def create_lindist_model(case: Case) -> pyo.ConcreteModel:
    ...
    m.v = pyo.Var(
        m.bus_phase_set, domain=pyo.NonNegativeReals
    )  # Voltage magnitude squared
    m.p_flow = pyo.Var(m.branch_phase_set)
    m.q_flow = pyo.Var(m.branch_phase_set)
    m.p_gen = pyo.Var(m.gen_phase_set, domain=pyo.NonNegativeReals)
    m.q_gen = pyo.Var(m.gen_phase_set)
    m.q_cap = pyo.Var(m.cap_phase_set)
    m.v_reg = pyo.Var(m.reg_phase_set, domain=pyo.NonNegativeReals)
