...
def add_regulator_constraints(m: pyo.ConcreteModel) -> None:
    ...
    def regulator_v_drop(m, _id, ph, t):
        raa = m.r[_id, ph + ph]
        xaa = m.x[_id, ph + ph]
        voltage_drop = 2 * raa * m.p_flow[_id, ph, t] + 2 * xaa * m.q_flow[_id, ph, t]
        return m.v[_id, ph, t] == m.v_reg[_id, ph, t] - voltage_drop
...
def add_cvr_load_constraints(m: pyo.ConcreteModel) -> None:
    ...
    def cvr_p_rule(m, _id, ph, t):
        p_nom = m.p_load_nom[_id, ph, t]
        cvr_p = m.cvr_p[_id, ph]
        return m.p_load[_id, ph, t] == p_nom + cvr_p * p_nom / 2 * (m.v[_id, ph, t] - 1)

    def cvr_q_rule(m, _id, ph, t):
        q_nom = m.q_load_nom[_id, ph, t]
        cvr_q = m.cvr_q[_id, ph]
        return m.q_load[_id, ph, t] == q_nom + cvr_q * q_nom / 2 * (m.v[_id, ph, t] - 1)
...
def add_swing_bus_constraints(m: pyo.ConcreteModel) -> None:
    ...
    def swing_voltage_rule(m, _id, ph, t):
