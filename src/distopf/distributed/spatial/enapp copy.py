...
class BoundaryVars:
    ...
    def _subtract_frame(
        ...
        diff = pd.merge(
            current, previous, how="left", on=["name", "t"], suffixes=("", "_prev")
        )
...
def parse_s_dn(case: Case, result: PowerFlowResult, down_buses: list):
    ...
    # Also handles parallel boundary branches.
...
def parse_v_dn(case: Case, result: PowerFlowResult, down_buses: list):
    ...
    return v.loc[
        v.name.astype(str).isin([str(name) for name in down_buses]),
        ["name", "t", "a", "b", "c"],
    ]
...
def parse_s_up(case: Case, result: PowerFlowResult):
    ...
    # Sum all branches leaving the swing bus.
...
def send_s_up(cases: dict[str, Case], boundaries: dict[str, BoundaryVars]):
    """Send each downstream area's upstream power to its upstream area."""
    for up_case in cases.values():
        for sending_area in up_case.bus_data.load_shape.astype(str):
            if sending_area not in cases:
                continue
...
def send_v_up(cases: dict[str, Case], boundaries: dict[str, BoundaryVars]):
    """Send each downstream area's upstream voltage to its upstream area."""
    for up_case in cases.values():
        for sending_area in up_case.bus_data.load_shape.astype(str):
            if sending_area not in cases:
                continue
            ...
            up_case.schedules = add_v_down_to_schedules(
                up_case.schedules, v_up, sending_area
            )

    return cases


def send_v_down(cases: dict[str, Case], boundaries: dict[str, BoundaryVars]):
    """Send each upstream area's downstream voltage to its downstream areas."""
    for boundary in boundaries.values():
        for down_name in boundary.v_down.name.astype(str).unique():
            if down_name not in cases:
                raise KeyError(f"Unknown downstream area: {down_name}")

            v_down = deepcopy(
                boundary.v_down.loc[boundary.v_down.name.astype(str) == down_name]
            )

            cases[down_name].schedules = add_v_swing_to_schedules(
                cases[down_name].schedules, v_down, down_name
            )

    return cases


def send_s_down(cases: dict[str, Case], boundaries: dict[str, BoundaryVars]):
    """Send each upstream area's downstream power to its downstream areas."""
    for sending_area, boundary in boundaries.items():
        for down_name in boundary.s_down.name.astype(str).unique():
            if down_name not in cases:
                raise KeyError(f"Unknown downstream area: {down_name}")

            s_down = deepcopy(
                boundary.s_down.loc[boundary.s_down.name.astype(str) == down_name]
            )

            cases[down_name].schedules = add_s_to_schedules(
                cases[down_name].schedules, s_down, sending_area
            )
...
def parse_all_boundaries(cases, all_results, area_info):
    boundaries = {}

    for area_name in cases:
        down_buses = area_info[area_name]["down_areas"]
        result = all_results[area_name]

        boundaries[area_name] = BoundaryVars(
            s_up=parse_s_up(cases[area_name], result),
            v_up=parse_v_up(cases[area_name], result),
            s_down=parse_s_dn(cases[area_name], result, down_buses),
            v_down=parse_v_dn(cases[area_name], result, down_buses),
        )
...
def send_all_boundaries(cases, boundaries):
    ...
    cases = send_v_up(cases, boundaries)
    cases = send_s_down(cases, boundaries)
...
def solve_enapp(
    ...
    **kwargs,
    ...
    for iterations in range(1, max_iterations + 1):
        ...
        boundaries = dampen_boundaries(boundaries, previous_boundaries, alpha=0.5)
